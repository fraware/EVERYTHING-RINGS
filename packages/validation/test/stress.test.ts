...
import { evaluateGateARelease, mergeValidationEvidence, parseValidationEvidence } from "../src";
...
describe("validation stress invariants", () => {
  it("accepts 1000 valid revisions and rejects malformed provenance", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      const bundle = evidence(`object-${seed}`, "metal", { sessionId: `session-${seed}`, specimenId: `specimen-${seed}`, softwareRevision });
      if (!parseValidationEvidence(bundle).ok) throw new Error(`seed ${seed}: valid evidence rejected`);
      if (parseValidationEvidence({ ...bundle, softwareRevision: `g${softwareRevision.slice(1)}` }).ok) throw new Error(`seed ${seed}: non-hex revision accepted`);
      if (parseValidationEvidence({ ...bundle, softwareRevision: softwareRevision.slice(0, 39) }).ok) throw new Error(`seed ${seed}: short revision accepted`);
    }
  });
  ...
  it("keeps 1000 same-revision Gate A releases valid and rejects a mixed revision", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      const bundles = fiveObjects().map((bundle, index) => ({ ...bundle, sessionId: `${bundle.sessionId}-${seed}-${index}`, softwareRevision }));
      const valid = evaluateGateARelease(bundles);
      if (!valid.passed || valid.softwareRevision !== softwareRevision) throw new Error(`seed ${seed}: homogeneous release did not pass`);
      const mixed = bundles.map((bundle, index) => index === 4 ? { ...bundle, softwareRevision: `f${softwareRevision.slice(1)}` === softwareRevision ? `e${softwareRevision.slice(1)}` : `f${softwareRevision.slice(1)}` } : bundle);
  ...
  it("merges 1000 repeated exports without changing immutable measurement provenance", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      const base = evidence(`merge-${seed}`, "ceramic", { sessionId: `merge-session-${seed}`, specimenId: `merge-specimen-${seed}`, softwareRevision });
