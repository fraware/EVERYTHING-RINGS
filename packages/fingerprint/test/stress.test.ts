...
describe("fingerprint recurrence stress invariants", () => {
  it("recovers 1000 global modal shifts under randomized candidate ordering", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      if (result.matchedCount !== 4 || result.unmatchedReferenceCount !== 0) {
        throw new Error(`seed ${seed}: recurrence failed one-to-one matching`);
      }
      if (Math.abs(result.medianCents - expectedDistance) > 1e-8) {
        throw new Error(`seed ${seed}: median ${result.medianCents} != ${expectedDistance}`);
      }
      if (Math.abs(result.meanCents - expectedDistance) > 1e-8) {
        throw new Error(`seed ${seed}: mean ${result.meanCents} != ${expectedDistance}`);
      }
      const candidateIndices = result.matches.map((match) => match.candidateIndex);
      if (new Set(candidateIndices).size !== 4) {
        throw new Error(`seed ${seed}: a candidate mode was reused`);
      }
    }
  });
});
