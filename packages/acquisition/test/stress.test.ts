  ...
  it("matches a reference queue across 1000 seeded ring-buffer state machines", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      for (let operation = 0; operation < operations; operation += 1) {
        ...
        if (ring.availableSamples !== expected.length) {
          throw new Error(`seed ${seed}, operation ${operation}: availableSamples ${ring.availableSamples} != ${expected.length}`);
        }
        ...
        if (actual.length !== wanted.length || actual.some((value, index) => value !== wanted[index])) {
          throw new Error(`seed ${seed}, operation ${operation}: ring-buffer content diverged`);
        }
      }
    }
  });
