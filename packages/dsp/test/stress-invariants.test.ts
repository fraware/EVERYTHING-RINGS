...
describe("DSP stress invariants", () => {
  it("recovers 1000 randomized parabolic spectral peaks", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      if (Math.abs(result.binOffset - expectedOffset) > 1e-9) {
        throw new Error(`seed ${seed}: peak offset ${result.binOffset} != ${expectedOffset}`);
      }
      if (Math.abs(result.magnitudeDb - peakDb) > 1e-8) {
        throw new Error(`seed ${seed}: interpolated magnitude ${result.magnitudeDb} != ${peakDb}`);
      }
      if (!(result.frequencyHz > 0) || !Number.isFinite(result.frequencyHz)) {
        throw new Error(`seed ${seed}: invalid interpolated frequency`);
      }
    }
  });
  ...
  it("recovers 1000 randomized robust lines with a single strong outlier", () => {
    for (let seed = 1; seed <= 1000; seed += 1) {
      ...
      if (!Number.isFinite(fit.slope) || !Number.isFinite(fit.intercept) || !Number.isFinite(fit.residualScale)) {
        throw new Error(`seed ${seed}: robust fit returned a non-finite value`);
      }
      if (Math.abs(fit.slope - expectedSlope) > 0.02) {
        throw new Error(`seed ${seed}: slope ${fit.slope} differs from ${expectedSlope}`);
      }
      if (!(fit.score >= 0 && fit.score <= 1)) {
        throw new Error(`seed ${seed}: fit score outside [0,1]`);
      }
    }
  });
});
