...
# Disposable hosted-v7 qualification. This branch is never merged.
LIVE_BASE_URL="https://fraware.github.io/EVERYTHING-RINGS"
EXPECTED_REVISION="ec591c1328f893da7cdcfccc69ed9233bcb69d9e"
...
if ! grep -Fq "software revision ${EXPECTED_REVISION}" /tmp/everything-rings-hosted-lab.html; then
  echo "Hosted lab does not report the frozen v7 revision" >&2
...
if ! grep -Fq "${EXPECTED_REVISION}" /tmp/everything-rings-hosted-campaign-author.html; then
  echo "Hosted campaign author does not report the frozen v7 revision" >&2
...
BROWSER="$BROWSER" BASE_URL="$LIVE_BASE_URL" node scripts/session-lifecycle-e2e.mjs
...
echo "Hosted v7 qualification passed."
