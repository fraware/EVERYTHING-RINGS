...
`analyzeImpact()` combines the accepted tracks into `AcousticFingerprintV1`. Confidence combines prominence, decay-fit quality, persistence, and frequency stability. A stable track must also reach at least `0.001` of the strongest accepted-track amplitude (-60 dB) to enter the reported modal set. This measurement floor prevents persistent numerical, device, or browser-noise tracks far below the observed response from being presented as audible resonances. Near-duplicate candidates are suppressed before ranking. The contract exposes at most 16 modes and requires at least three to call a capture a stable fingerprint.
