...
Only objects with a passing Gate A2 session are eligible. Gate A2 passes only after exactly five qualified attempts all succeed under the frozen physical contract. The Gate B review target is the **fifth qualified attempt** of one passing Gate A2 session. Each review stores the object label, session ID, and attempt ID; a review targeting another session, another attempt, or an analytically failed attempt does not count.

If an object has more than one passing Gate A2 session, all counted Gate B reviewers for that object must hear the **same single session/attempt target**. Reviews from different passing sessions are never pooled. If submitted Gate B reviews for one object span multiple eligible targets, that object's Gate B state remains OPEN.
...
Gate B passes when at least five Gate A2 objects have eligible blinded reviews, at least four objects pass, and the passing reconstruction set contains metal, glass, and ceramic examples.
